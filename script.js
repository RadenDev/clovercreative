...
const ArtBot = {
  ...
  *Catatan*:
  ...
  generateResponse(userInput) {
    ...
    const { service, type, action, size, item, other } =
  ...
  handlePriceRequest() {
    ...
    if (this.currentTopic !== "custom" && !this.pendingData.type) {
      return `Untuk menghitung harga ${
        this.currentTopic
      }, sebutkan jenis lukisannya (${Object.keys(service.types).join("/")}).`;
    }

    if (this.currentTopic !== "custom" && !this.pendingData.size) {
      return `Untuk ${this.currentTopic} ${
        this.pendingData.type || ""
      }, sebutkan ukuran (contoh: 2x3 meter).`;
    }
    ...
    if (this.currentTopic === "mural") {
      ...
      response += `- Lukisan: Rp${service.types[
        this.pendingData.type
      ].price.toLocaleString()}/m² × ${area} m² = Rp${artPrice.toLocaleString()}\n`;
  ...
  handleInfoRequest() {
    ...
    if (this.currentTopic !== "custom") {
      ...
      for (const [type, info] of Object.entries(service.types)) {
        response += `- ${type}: ${info.desc} (${
          services[this.currentTopic].pricing
        })\n`;
      }
...
setTimeout(() => {
  addMessage(
    "🎨 *Selamat datang di Jasa Lukis CloverArt* 🎨\n\n" +
